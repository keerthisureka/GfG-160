...
class Solution {
    ...
    static void computeLPSArray(String s, int[] lps) {
        ...
        // lps[0] is always 0
        lps[0] = 0;
        // The loop calculates lps[i] for i = 1 to str.length() - 1
        while(idx < s.length()) {
            ...
            else { // If len is 0, then we have no common prefix which is also a suffix
                if(len == 0) {
                    lps[idx] = 0;
                    idx++;
                } 
                else { // Note that we do not move to the next index
                    len = lps[len - 1];
                }
            }
        }
    ...
    static boolean KMPSearch(String txt, String pat, int[] lps, int rep) {
        ...
        while(i < n * rep) {
            ...
            else { // If there is a mismatch Use lps value of previous index to avoid redundant comparisons
                if(j != 0)
                    j = lps[j - 1];
                else
                    i++;
    ...
    static int minRepeats(String s1, String s2) {
        ...
        // Find the minimum number of times s1 needs to be repeated to become as long as s2
        int x = (m + n - 1) / n;
